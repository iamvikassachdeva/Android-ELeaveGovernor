...
public class MarkAttendence extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!fingerprintManager.isHardwareDetected())
            ViewDialogBox.showDialogBox("Fingerprint authentication permission not enable",this);
            //Toast.makeText(this, "Fingerprint authentication permission not enable", Toast.LENGTH_SHORT).show();
        else {
            if (!fingerprintManager.hasEnrolledFingerprints())
                ViewDialogBox.showDialogBox("Register atleast one fingerprint in setting",this);
                //Toast.makeText(this, "Register atleast one fingerprint in setting", Toast.LENGTH_SHORT).show();

            else {
                if (!keyguardManager.isKeyguardSecure())
                    ViewDialogBox.showDialogBox("Lock screen security not enabled is settings",this);
                    //Toast.makeText(this, "Lock screen security not enable is settings", Toast.LENGTH_SHORT).show();
                else
                    genKey();
